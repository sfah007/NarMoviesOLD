...
let movies = JSON.parse(fs.readFileSync("Databases/EgybestAllMoviesV3.json"));
...
app.get("/home", function (req, res) {
  res.render("homeV", { data: GetHomeData(), featured: featured["featured"] });
});
...
app.get("/movie/:id", function (req, res) {
  ...
  GetBackDrop(title).then((backdrop) =>
    GetStreamLinks(data["str"]).then((sources) =>
      res.render("movieV", { movie: data, backdrop: backdrop, streams: sources })
    )
  );
});
...
function GetHomeData() {
  ...
  for (let i = 0; i < movies.length; i++) {
    ...
    if (
      movies[i]["genres"]["ar"].includes("اكشن") &&
      data["movies-action"].length <= 20
    ) {
    ...
    if (
      movies[i]["genres"]["ar"].includes("دراما") &&
      data["movies-drama"].length <= 20
    ) {
    ...
    if (
      movies[i]["genres"]["ar"].includes("كرتون") &&
      data["movies-animation"].length <= 20
    ) {
...
function GetmoviesByGenre() {
  return (req, res, next) => {
    ...
    for (let i = 0; i < movies.length; i++) {
      if (movies[i]["genres"]["ar"].includes(req.query.genre)) {
